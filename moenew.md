...
* 如何获取api：
  ...
  * 免费服务[Pollinations.AI](https://pollinations.ai/)：开源平台，官方称提供简单易用的文本和图像生成API，保障用户隐私。选择OpenAI兼容类型，填写URL为`https://gen.pollinations.ai/v1`。要使用更多模型，前往[验证页面](https://enter.pollinations.ai/sign-in)连接你的GitHub账号并生成密钥。现在有DeepSeek V3.2、Claude Haiku 4.5、Kimi K2.5、GLM-5等优质模型。 *可能有严格的审核，并且对话内容可能会在pollinations上展示*
  ...
  * 其它送免费额度的提供商，在 "API设置" 选择 OpenAI兼容类型，粘贴 Key 并填写模型名。

  **B. 使用付费模型**

  1. 可以使用OpenAI兼容格式的URL。例如：
  ...
  * 在 "API设置" 选择 OpenAI兼容类型，粘贴 api Key到“API密钥”、获取模型后可点击“测试连接”验证api配置是否有效。

  2. 其它类型api，请使用api模板：[![API模板](https://img.shields.io/badge/文档-模板引擎-blue.svg)](https://github.com/fatsnk/APItemplateV2?tab=readme-ov-file)

* 配置完毕后，点击底部的“保存设置”关闭设置窗口。<br>
*2026年6月推荐模型：Claude 4.6/4.8 opus、gemini 3.1 pro、glm4.6~glm5.1、deepseek V4 pro;备选：Gemini 2.5 pro、、MiniMax M3、Qwen3.7-Max*
...
|persona description|你的用户提示词，在“YOU”中填写。|你自己的人设，用于告诉AI“用户是谁，用户扮演什么角色”。但这只是口头约定，实际上可以填写任意文本|
...
注意：填写的提示词并没有严格限制，系统也不会告诉模型“这里是角色描述、那里是世界书”；
可以试着自己填写，然后在app右上角的预览提示词中观察发送的内容，检查预览中的文本，思考一下：1.文本顺序有没有问题？ 2.假设你是AI,能不能看懂你填写的提示词、能不能理解你的需求？
...
修改对话内容（中文引号包裹的内容）的文字颜色：

<img src="./images/对话颜色正则.jpg" width="300"/>

如图，使用正则`([“「])([^“”「」]*?)([”」])`匹配中文引号及其包裹的内容，然后替换成你喜欢的颜色，例如`<font color='pink'>“$2”</font>`。根据你的需求可以改成其它颜色值。

*（此示例仅用于展示正则脚本的使用方法，如果你需要修改对话颜色，无需使用正则，直接在主题与样式设置中的符号选项卡添加符号规则即可。）*
...
替换为：`<i><span style="color: #CC99FF;">$1</span></i>`

选项勾选修改聊天显示即可。color可以替换为你喜欢的颜色值。
...
[![宏](https://img.shields.io/badge/文档-宏与变量-blue.svg)](https://github.com/fatsnk/forksilly.doc/blob/main/%E5%AE%8F%E5%BC%95%E6%93%8E%E6%B5%8B%E8%AF%95%E7%94%A8%E4%BE%8B.md)
...
<img src="https://github.com/user-attachments/assets/c7ddffbf-d18a-403a-bf81-31b20b64d222" width="300"/>
