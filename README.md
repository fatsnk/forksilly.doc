...
* **适合纯文字卡、插图卡（使用图床或[本地上传](#使用技巧)），以及Stable Diffusion文生图。不适合花里胡哨的前端卡**
+ 兼容sillytavern V2角色卡（png）、世界书、正则、预设、聊天记录（[兼容性指引](#兼容性指引)）
  - 暂不支持新建角色，请通过导入空角色卡的方式创建角色，或者使用[角色卡编辑器](#推荐工具)
+ 支持更换任意字体，将字体文件下载到你的手机然后在应用中选择即可
+ 支持更换聊天背景图片
+ 可导入导出聊天记录、新增/编辑/导出角色世界书/正则、管理/启用禁用多个全局世界书/正则、支持编辑/新增/导入导出全局世界书和正则脚本
+ 聊天时快速开关预设条目
...
*以上注意事项均是特性，不是bug;不要问能不能改，请阅读[ForkSilly介绍](#ForkSilly介绍)第一条的内容* （
  - 已知“特性”：现在屏幕上显示的消息不会即时计算全局正则脚本的“最小深度”和“最大深度”，可通点击聊天历史📓按钮选择当前聊天记录来刷新。（该“特性”不影响“修改请求内容”设置，发送给API的提示词会立即应用正则的深度。）不鼓励依赖太多正则，瞎写的正则很容易引起回溯，造成严重性能问题；添加过多的正则脚本更容易产生问题。
