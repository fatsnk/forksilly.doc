...
| 处理后移除调用标签 | 全部 | 工具执行完毕会从消息中移除 invoke 标签（推荐关闭） |
| 结果处理模式 | 仅 MCP 工具 | 「追加到消息末尾」或「覆盖整条消息」（推荐“追加”） |
...
批量调用中的工具会按顺序依次执行，上下文会传递（仅mcp工具），但批量调用整体不会触发递归（即结果中的新 invoke 标签不会被继续处理）。

---

### 八、提示词示例

这个场景其实就是给 LLM 一段工具使用提示词，告诉模型什么时候使用工具、如何使用工具。提示词可以添加在app中的预设、世界书、角色卡等任何可以注入提示词的地方。
可以参考以下示例：
...
<invoke name="BaiduAIsearch/chatCompletions">
{
  "query": "搜索关键词"
}
</invoke>
...
- 检查「结果处理模式」设置是否符合预期
- 检查「处理后移除调用标签」是否开启
- 检查模型的思维链，是否思维链中可能出现的invoke标签导致意外触发工具（可关闭api设置中的思维链显示开关，该开关不会影响模型思考，只是模型的思考过程不返回给前端）
