...
*   `package.json`: Node.js 项目清单文件。定义项目依赖、脚本（如启动、构建）和基本信息。
...
*   **`components/`**: 可重用的 UI 组件。
    ...
    *   `CharacterBubbleSelector.tsx`: **角色气泡选择器组件**。提供一个全屏的、带有动画效果的角色选择界面。当触发时，会从屏幕上的一个点“吹出”多个角色气泡，每个气泡代表一个可选角色。气泡会以动画形式移动到屏幕的随机位置，并带有轻微的漂浮和旋转效果。**优化：通过智能分布算法，确保生成的气泡目标位置不会相互重叠，并且会避开顶部的状态栏和底部的输入框区域，以提供清晰的视觉效果。**
    ...
    *   `CharacterDetailModal.tsx`: **角色详情模态框**。用于显示角色的信息。
    *   `MemoryModal.tsx`: 角色记忆模态框，管理当前角色的记忆。
*   **`context/`**: React Context API 相关文件。
    *   `ModalContext.tsx`: 全局模态框上下文。定义并提供 `ModalProvider`，用于在应用顶层集中管理所有全局模态框的状态。它维护着当前哪个模态框可见 (`visibleModal`) 以及该模态框所需的全部 `props` (`modalProps`)，并提供 `showModal(modal, props)` 和 `hideModal()` 方法供应用内任何组件调用(需优化)。
...
*   **`hooks/`**: 存放自定义 React Hooks，封装可重用的状态逻辑。
    *   `useAnimatedModal.ts`: **动画模态框 Hook**。为全局模态框提供统一的动画逻辑。它接收一个 `visible` 布尔值，并使用 `react-native-reanimated` 的 `useSharedValue`、`withTiming` 和 `withSpring`，返回可直接应用于视图的动画样式（`backdropAnimatedStyle` 用于背景遮罩，`modalAnimatedStyle` 用于模态框内容），实现了平滑的淡入/缩放动画效果。
    *   `useMessageActions.ts`: **(重构) 消息操作 Hook**。封装了与聊天消息相关的操作逻辑。
    *   `useSlideUpModal.ts`: **向上弹出模态框动画 Hook**。为预设条目开关等模态框提供从底部向上滑入的动画效果。使用 `react-native-reanimated` 的 `useSharedValue`、`withTiming` 和 `withSpring` 实现平滑的向上滑动动画，支持背景遮罩的透明度变化和模态框的垂直位移动画，适用于需要从屏幕底部弹出的模态框组件。
    *   `useStreamAnimator.ts`: **(修复) 流式动画 Hook**。支持自适应速度调整。
    *   `useBrowserStorage.ts`: **浏览器存储 Hook**。提供一些内置浏览器使用的函数。
    *   `useSlashCommands.ts`: **斜杠命令 Hook**。用于处理聊天输入框中的斜杠命令。
    *   `useChatScreenHandlers.ts`: **聊天屏幕处理器 Hook**。包含 `handlePreviewPrompt` 和 `handleMemoryExtractionAndSaving` 等辅助函数。
...
*   **`screens/`**: 应用的主要屏幕或页面。
    ...
    *   `TextToImageSettingsScreen.tsx`: **文生图设置屏幕**。允许用户配置文生图相关的设置，例如管理文生图API配置、管理文生图预设，以及设置文生图的触发机制（如自定义触发标签、图片插入位置等）。依赖 `imageGenerationPresetService.ts` 和 `imageTriggerService.ts`。
    *   `MemoryAndKnowledgeScreen.tsx`: **记忆与知识库设置屏幕**。用于配置记忆和知识库的相关参数。
*   **`services/`**: 封装应用的业务逻辑、数据处理和与外部服务的交互。
    ...
    *   `stableDiffusionService.ts`: **Stable Diffusion 服务**。封装了与 Stable Diffusion API（或其他兼容的文生图API）的交互逻辑。负责根据提供的API配置和参数（如提示词、尺寸、步数等）发送请求，并处理返回的图像数据。
    ...
    *   `streamAnimationService.ts`: **流式动画服务**。提供发布-订阅模式的流式数据分发服务，用于接收 ChatScreen 传递的流式数据并分发给多个动画器。实现了消息级别的订阅管理，支持 `subscribe`（订阅指定消息ID的数据流）、`unsubscribe`（取消订阅）、`emit`（发送数据块）和 `forceComplete`（强制完成动画）等核心方法，确保流式动画的数据传递和状态同步。
    ...
    *   `dynamicContextService.ts`: **动态上下文服务**。负责构建动态插入到上下文中的记忆和知识库。
    *   `embeddingApiService.ts`: **嵌入式模型API服务**。用于处理嵌入式模型的API请求。
    *   `embeddingSettingsService.ts`: **嵌入式模型设置服务**。管理嵌入式模型API配置以及记忆与知识库的参数。
    *   `imageGenApiConfigService.ts`: **文生图API配置服务**。负责管理文生图API的配置文件。
    *   `knowledgeService.ts`: **数据库操作服务**。封装了处理记忆和知识库相关的表的操作。
    *   `presetVariableService.ts`: 用于预设编辑屏幕的“处理预设变量”按钮。将预设提示词中的“{{setvar}}”等占位符替换为实际的提示词。
*   **`types/`**: TypeScript 类型定义文件。
    ...
    *   `imageGeneration.ts`: **文生图类型定义**。定义了与文生图功能相关的各种TypeScript数据结构，例如API配置 (`ImageGenApiConfig`)、预设 (`ImageGenerationPreset`)、预设中的提示词片段 (`ImagePromptSnippet`)、Stable Diffusion API参数 (`SdTextToImageParams`) 以及图片触发设置 (`ImageTriggerSettings`) 等。
    ...
    *   `op-sqlite.d.ts`: **op-SQLite类型定义文件**。为 op-SQLite 数据库提供 TypeScript 类型声明。
*   **`utils/`**: 包含通用的辅助函数或工具类。
...
*Total : 183 files, 56233 codes, 6117 comments, 6063 blanks, all 68413 lines*

|filename|language|code|comment|blank|total|
|:----|:----|:----|:----|:----|:----|
|App.tsx|TypeScript JSX|203|13|17|233|
...
|android/app/src/main/java/com/anonymous/forksilly/MyForegroundService.kt|Kotlin|72|10|12|94|
...
|modules/fileprocess/android/src/main/java/expo/modules/fileprocess/ChatUtils.kt|Kotlin|296|77|60|433|
|modules/fileprocess/android/src/main/java/expo/modules/fileprocess/FileprocessModule.kt|Kotlin|257|44|54|355|
...
|modules/fileprocess/src/FileprocessModule.ts|TypeScript|33|14|8|55|
...
|src/components/AnimatedChatHistoryModal.tsx|TypeScript JSX|1,095|17|58|1,170|
|src/components/AnimatedScreenView.tsx|TypeScript JSX|32|1|7|40|
|src/components/AnimatedStreamedMessage.tsx|TypeScript JSX|543|47|62|652|
|src/components/BackupManagerModal.tsx|TypeScript JSX|703|38|80|821|
|src/components/CharacterBubbleSelector.tsx|TypeScript JSX|409|15|52|476|
|src/components/CharacterDetailModal.tsx|TypeScript JSX|293|6|29|328|
|src/components/ChatDialog.tsx|TypeScript JSX|1,714|245|172|2,131|
|src/components/ChatHistoryModal.tsx|TypeScript JSX|571|19|35|625|
|src/components/ChatInput.tsx|TypeScript JSX|607|34|50|691|
|src/components/ColorPickerModal.tsx|TypeScript JSX|488|18|41|547|
...
|src/components/ImagePreviewModal.tsx|TypeScript JSX|342|4|36|382|
|src/components/MacroVariableModal.tsx|TypeScript JSX|861|15|42|918|
|src/components/MemoryModal.tsx|TypeScript JSX|1,540|57|108|1,705|
|src/components/ModelSelectorModal.tsx|TypeScript JSX|305|10|13|328|
|src/components/MoreSettingsModal.tsx|TypeScript JSX|174|8|13|195|
|src/components/PresetEntriesToggleModal.tsx|TypeScript JSX|224|1|12|237|
|src/components/PresetSelector.tsx|TypeScript JSX|52|1|6|59|
|src/components/ProfileManagerPanel.tsx|TypeScript JSX|1,173|64|84|1,321|
|src/components/PromptListItem.tsx|TypeScript JSX|531|15|40|586|
|src/components/PromptPreviewModal.tsx|TypeScript JSX|627|13|57|697|
|src/components/SaveAsModal.tsx|TypeScript JSX|151|1|9|161|
|src/components/SideMenu.tsx|TypeScript JSX|481|13|41|535|
|src/components/SimpleBrowserModal.tsx|TypeScript JSX|679|5|51|735|
|src/components/TopBar.tsx|TypeScript JSX|308|16|20|344|
|src/components/WelcomeScreen.tsx|TypeScript JSX|569|26|62|657|
|src/components/WorldBookImportModals.tsx|TypeScript JSX|462|10|29|501|
...
|src/context/ThemeContext.tsx|TypeScript JSX|404|44|42|490|
|src/hooks/useAIResponsePostProcess.ts|TypeScript|78|15|6|99|
...
|src/hooks/useImageGeneration.ts|TypeScript|405|37|72|514|
|src/hooks/useMessageActions.ts|TypeScript|153|10|21|184|
...
|src/hooks/useStreamAnimator.ts|TypeScript|204|20|43|267|
|src/hooks/useWhyDidYouUpdate.ts|TypeScript|23|13|7|43|
|src/navigation/NavigationService.ts|TypeScript|14|1|2|17|
|src/screens/CharacterEditScreen.tsx|TypeScript JSX|2,060|117|144|2,321|
|src/screens/CharacterManagementScreen.tsx|TypeScript JSX|1,472|177|178|1,827|
|src/screens/ChatScreen.tsx|TypeScript JSX|1,615|231|228|2,074|
|src/screens/GlobalRegexEditScreen.tsx|TypeScript JSX|543|25|41|609|
|src/screens/GlobalRegexManagementScreen.tsx|TypeScript JSX|302|28|20|350|
|src/screens/GlobalWorldBookEditScreen.tsx|TypeScript JSX|1,048|111|65|1,224|
|src/screens/GlobalWorldBookManagementScreen.tsx|TypeScript JSX|470|74|35|579|
|src/screens/MemoryAndKnowledgeScreen.tsx|TypeScript JSX|1,960|24|135|2,119|
|src/screens/PersonaManagementScreen.tsx|TypeScript JSX|701|37|43|781|
|src/screens/PresetEditScreen.tsx|TypeScript JSX|1,092|98|138|1,328|
|src/screens/PresetManagementScreen.tsx|TypeScript JSX|739|59|64|862|
|src/screens/SettingsScreen.tsx|TypeScript JSX|1,344|87|75|1,506|
|src/screens/StorageManagementScreen.tsx|TypeScript JSX|962|21|58|1,041|
|src/screens/TextToImageSettingsScreen.tsx|TypeScript JSX|1,763|129|133|2,025|
|src/screens/ThemeSettingsScreen.tsx|TypeScript JSX|150|2|22|174|
|src/screens/ThemeSettingsTabs/CustomTagsTab.tsx|TypeScript JSX|491|6|35|532|
|src/screens/ThemeSettingsTabs/FontSettingsTab.tsx|TypeScript JSX|346|1|20|367|
|src/screens/ThemeSettingsTabs/ThemeSettingsTab.tsx|TypeScript JSX|916|2|48|966|
|src/screens/ToolSettingsScreen.tsx|TypeScript JSX|1,208|53|65|1,326|
|src/services/apiConfigService.ts|TypeScript|229|31|34|294|
|src/services/backupService.ts|TypeScript|468|75|81|624|
...
|src/services/globalWorldBookService.ts|TypeScript|522|120|86|728|
...
|src/services/macroEngine/MacroCstWalker.ts|TypeScript|533|3|87|623|
|src/services/macroEngine/MacroDiagnostics.ts|TypeScript|61|0|11|72|
|src/services/macroEngine/MacroEngine.ts|TypeScript|208|0|35|243|
|src/services/macroEngine/MacroEnv.types.ts|TypeScript|42|0|6|48|
|src/services/macroEngine/MacroFlags.ts|TypeScript|118|0|11|129|
|src/services/macroEngine/MacroLexer.ts|TypeScript|192|14|21|227|
|src/services/macroEngine/MacroParser.ts|TypeScript|162|0|28|190|
|src/services/macroEngine/MacroRegistry.ts|TypeScript|339|0|43|382|
|src/services/macroEngine/MacroVariableService.ts|TypeScript|461|19|88|568|
|src/services/macroEngine/SideEffectProcessor.ts|TypeScript|8|9|3|20|
|src/services/macroEngine/definitions/coreMacros.ts|TypeScript|299|42|42|383|
|src/services/macroEngine/definitions/timeMacros.ts|TypeScript|136|4|15|155|
|src/services/macroEngine/definitions/variableMacros.ts|TypeScript|231|1|16|248|
|src/services/mcpService.ts|TypeScript|246|60|57|363|
|src/services/openAIService.ts|TypeScript|1,311|281|183|1,775|
|src/services/personaService.ts|TypeScript|258|40|39|337|
|src/services/placeholderService.ts|TypeScript|163|83|38|284|
|src/services/presetService.ts|TypeScript|302|46|45|393|
|src/services/presetVariableService.ts|TypeScript|78|16|11|105|
|src/services/profileService.ts|TypeScript|556|91|76|723|
|src/services/promptPostProcessorService.ts|TypeScript|78|29|17|124|
|src/services/regexApplyService.ts|TypeScript|37|18|11|66|
|src/services/regexScriptService.ts|TypeScript|297|66|46|409|
|src/services/sillyTavernContextBuilder.ts|TypeScript|360|244|82|686|
|src/services/stableDiffusionService.ts|TypeScript|303|63|40|406|
...
|src/services/templateEngine/mediaService.ts|TypeScript|393|152|74|619|
|src/services/templateEngine/requestBuilder.ts|TypeScript|168|91|40|299|
|src/services/templateEngine/responseParser.ts|TypeScript|228|76|41|345|
|src/services/templateEngine/templateLoader.ts|TypeScript|197|80|40|317|
|src/services/templateEngine/transportLayer.ts|TypeScript|574|96|78|748|
|src/services/toolExecutionService.ts|TypeScript|341|118|61|520|
|src/services/toolSettingsService.ts|TypeScript|155|20|19|194|
|src/services/worldBookService.ts|TypeScript|412|161|81|654|
|src/styles/CharacterManagementScreen.styles.ts|TypeScript|480|42|15|537|
|src/styles/ChatDialog.styles.ts|TypeScript|373|64|2|439|
|src/styles/ChatScreen.styles.ts|TypeScript|171|1|1|173|
|src/styles/ThemeSettings.styles.ts|TypeScript|389|0|1|390|
|src/styles/tableStyles.ts|TypeScript|34|1|1|36|
|src/types/apiTypes.ts|TypeScript|79|17|16|112|
